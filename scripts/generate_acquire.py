...
PAGE_COPY: Dict[str, Dict[str, Any]] = {
    "en": {
        ...
        "asset_body": "TourVsTravel.com is a governed multilingual reference system for Travel Decision Architecture. It publishes the Travel Structure Ontology (TSO) v1 — seventeen travel structures profiled on six declared axes — the Travel Decision Integrity Standard (TDIS) v1 with its Structure Fit Protocol, an operating client-side diagnostic engine (the Travel Decision Compass), versioned machine-readable JSON endpoints for AI agents, and a public governance record: an append-only changelog and decision log. Every build is contract-verified in seven languages, with link integrity, claims restraint, and localization enforced by gates that fail the build.",
        ...
        "included_body": "The acquisition includes: the domain name (tourvstravel.com); the category infrastructure — TSO v1, TDIS v1, and the Structure Fit Protocol, with their canonical pages and versioned JSON artifacts; the Travel Decision Compass engine; the static site codebase including Python generators, Jinja2 templates, and the gated build pipeline; the structured multilingual content in seven languages; the public governance layer (standing rules, decision log, changelog); and the GitHub repository as structured.",
    ...
    "ar": {
        ...
        "asset_label": "الأصل",
        "asset_body": "TourVsTravel.com نظام مرجعي محكوم متعدد اللغات لبنية قرار السفر. ينشر أنطولوجيا بنى السفر (TSO) الإصدار الأول — سبع عشرة بنية سفر موصوفة على ستة محاور معلنة — ومعيار سلامة قرار السفر (TDIS) الإصدار الأول مع بروتوكول ملاءمة البنية، ومحرك تشخيص تشغيلي يعمل في المتصفح (بوصلة قرار السفر)، ونقاط وصول JSON مُصدَّرة مقروءة آليًا لوكلاء AI، وسجل حوكمة علني: سجل تغييرات وسجل قرارات لا يقبلان إلا الإضافة. كل بناء يُتحقق من عقده بسبع لغات، مع بوابات تُفشل البناء عند كسر الروابط أو خرق ضبط الادعاءات أو نقص الترجمة.",
        ...
        "included_body": "يشمل الاستحواذ: اسم النطاق (tourvstravel.com)؛ والبنية التحتية الفئوية — TSO v1 وTDIS v1 وبروتوكول ملاءمة البنية بصفحاتها القانونية ووثائق JSON المُصدَّرة؛ ومحرك بوصلة قرار السفر؛ وقاعدة كود الموقع الثابت بما فيها مولدات Python وقوالب Jinja2 وخط البناء المبوَّب؛ والمحتوى المنظم متعدد اللغات بسبع لغات؛ وطبقة الحوكمة العلنية (القواعد الدائمة وسجل القرارات وسجل التغييرات)؛ ومستودع GitHub كما هو منظم.",
    ...
    "fr": {
        ...
        "asset_label": "L'actif",
        "asset_body": "TourVsTravel.com est un système de référence gouverné et multilingue pour l'architecture de décision de voyage. Il publie la Travel Structure Ontology (TSO) v1 — dix-sept structures de voyage profilées sur six axes déclarés —, le Travel Decision Integrity Standard (TDIS) v1 avec son Structure Fit Protocol, un moteur de diagnostic opérationnel côté navigateur (la Boussole de Décision de Voyage), des points d'accès JSON versionnés lisibles par les agents IA, et un registre de gouvernance public : journal des modifications et journal de décisions en ajout seul. Chaque build est vérifié par contrat en sept langues, avec des barrières qui font échouer le build en cas de lien brisé, d'affirmation non étayée ou de localisation manquante.",
        ...
        "included_body": "L'acquisition comprend : le nom de domaine (tourvstravel.com) ; l'infrastructure de catégorie — TSO v1, TDIS v1 et le Structure Fit Protocol, avec leurs pages canoniques et artefacts JSON versionnés ; le moteur Boussole de Décision de Voyage ; la base de code du site statique incluant les générateurs Python, les modèles Jinja2 et le pipeline de build à barrières ; le contenu multilingue structuré en sept langues ; la couche de gouvernance publique (règles permanentes, journal de décisions, journal des modifications) ; et le dépôt GitHub tel que structuré.",
    ...
    "es": {
        ...
        "asset_body": "TourVsTravel.com es un sistema de referencia gobernado y multilingüe para la arquitectura de decisión de viaje. Publica la Travel Structure Ontology (TSO) v1 — diecisiete estructuras de viaje perfiladas en seis ejes declarados —, el Travel Decision Integrity Standard (TDIS) v1 con su Structure Fit Protocol, un motor de diagnóstico operativo en el navegador (la Brújula de Decisión de Viaje), puntos de acceso JSON versionados legibles por agentes de IA, y un registro público de gobernanza: un registro de cambios y un registro de decisiones de solo adición. Cada build se verifica por contrato en siete idiomas, con barreras que hacen fallar el build ante enlaces rotos, afirmaciones sin respaldo o localización incompleta.",
        ...
        "included_body": "La adquisición incluye: el nombre de dominio (tourvstravel.com); la infraestructura de categoría — TSO v1, TDIS v1 y el Structure Fit Protocol, con sus páginas canónicas y artefactos JSON versionados; el motor Brújula de Decisión de Viaje; la base de código del sitio estático con generadores Python, plantillas Jinja2 y el pipeline de build con barreras; el contenido multilingüe estructurado en siete idiomas; la capa pública de gobernanza (reglas permanentes, registro de decisiones, registro de cambios); y el repositorio de GitHub tal como está estructurado.",
    ...
    "de": {
        ...
        "asset_body": "TourVsTravel.com ist ein regelgeleitetes, mehrsprachiges Referenzsystem für Reiseentscheidungsarchitektur. Es veröffentlicht die Travel Structure Ontology (TSO) v1 — siebzehn Reisestrukturen, profiliert auf sechs deklarierten Achsen —, den Travel Decision Integrity Standard (TDIS) v1 mit seinem Structure Fit Protocol, eine operative Diagnose-Engine im Browser (den Reise-Entscheidungskompass), versionierte maschinenlesbare JSON-Endpunkte für KI-Agenten und ein öffentliches Governance-Register: Änderungsprotokoll und Entscheidungsprotokoll, beide append-only. Jeder Build wird in sieben Sprachen vertraglich verifiziert; Gates lassen den Build bei gebrochenen Links, unbelegten Aussagen oder fehlender Lokalisierung fehlschlagen.",
        ...
        "included_body": "Der Erwerb umfasst: den Domainnamen (tourvstravel.com); die Kategorie-Infrastruktur — TSO v1, TDIS v1 und das Structure Fit Protocol mit ihren kanonischen Seiten und versionierten JSON-Artefakten; die Kompass-Engine; die statische Site-Codebasis mit Python-Generatoren, Jinja2-Templates und der Gate-gesicherten Build-Pipeline; den strukturierten mehrsprachigen Inhalt in sieben Sprachen; die öffentliche Governance-Ebene (ständige Regeln, Entscheidungsprotokoll, Änderungsprotokoll); sowie das GitHub-Repository in seiner strukturierten Form.",
    ...
    "zh": {
        ...
        "asset_label": "资产描述",
        "asset_body": "TourVsTravel.com 是一个受治理的多语言旅行决策架构参考系统。它发布旅行结构本体（TSO）v1——在六个明确轴上刻画的十七种旅行结构——以及含结构适配协议的旅行决策完整性标准（TDIS）v1、一个在浏览器中运行的诊断引擎（旅行决策指南针）、供 AI 代理读取的版本化 JSON 端点，以及公开的治理记录：只增不改的更新日志与决策日志。每次构建都以七种语言进行契约验证，链接完整性、声明约束与本地化均由使构建失败的关卡强制执行。",
        "name_label": "名称价值",
        ...
        "included_label": "包含内容",
        "included_body": "收购包含：域名（tourvstravel.com）；类别基础设施——TSO v1、TDIS v1 与结构适配协议及其规范页面和版本化 JSON 工件；旅行决策指南针引擎；静态网站代码库，含 Python 生成器、Jinja2 模板与带关卡的构建管道；七种语言的结构化多语言内容；公开治理层（常设规则、决策日志、更新日志）；以及结构化的 GitHub 仓库。",
    ...
    "ja": {
        ...
        "asset_label": "アセットの説明",
        "asset_body": "TourVsTravel.com は、旅行意思決定アーキテクチャのための統治された多言語リファレンスシステムである。旅行構造オントロジー（TSO）v1——6つの宣言された軸でプロファイルされた17の旅行構造——、構造適合プロトコルを含む旅行意思決定インテグリティ基準（TDIS）v1、ブラウザ内で動作する診断エンジン（トラベル・ディシジョン・コンパス）、AIエージェント向けのバージョン管理された JSON エンドポイント、そして公開ガバナンス記録（追記専用の変更履歴と意思決定ログ）を公開している。すべてのビルドは7言語で契約検証され、リンク整合性、クレーム抑制、ローカライズはビルドを失敗させるゲートで強制される。",
        "name_label": "名前の価値",
        ...
        "included_body": "取得には以下が含まれる：ドメイン名（tourvstravel.com）；カテゴリーインフラ——TSO v1、TDIS v1、構造適合プロトコルとその正規ページおよびバージョン管理された JSON アーティファクト；コンパスエンジン；Python ジェネレーター、Jinja2 テンプレート、ゲート付きビルドパイプラインを含む静的サイトコードベース；7言語の構造化多言語コンテンツ；公開ガバナンス層（常設ルール、意思決定ログ、変更履歴）；および構造化された GitHub リポジトリ。",
