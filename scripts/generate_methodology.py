#!/usr/bin/env python3
"""
TourVsTravel — Methodology Page Generator
========================================

Purpose
-------
Generate the multilingual methodology page into:

    output/{lang}/methodology/index.html
...
- no partial writes
- multilingual-safe
- same SEO architecture as the rest of the system
- official route comes from scripts.routes.build_methodology_path()

Execution
---------
Run from repository root:

    python -m scripts.generate_methodology
    python -m scripts.generate_methodology --lang en
"""
...
log = logging.getLogger("generate_methodology")
...
class GenerateMethodologyError(Exception):
    """Base methodology generator error."""


class ConfigError(GenerateMethodologyError):
    """Raised when site configuration is missing or invalid."""


class AssetError(GenerateMethodologyError):
    """Raised when required static assets are missing or invalid."""


class RenderError(GenerateMethodologyError):
...
def _ensure_mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise GenerateMethodologyError(f"{label} must be a mapping/object.")
    return value


def _ensure_string(value: Any, label: str, allow_empty: bool = False) -> str:
    if not isinstance(value, str):
        raise GenerateMethodologyError(f"{label} must be a string.")
    text = value.strip()
    if not allow_empty and not text:
        raise GenerateMethodologyError(f"{label} must not be empty.")
    return text
...
def _translate_node(
    ...
    raise GenerateMethodologyError(f"{field_name} must be either a string or multilingual mapping.")
...
def _atomic_write_text(path: Path, content: str) -> None:
    ...
    try:
        ...
        if tmp_path is None:
            raise GenerateMethodologyError(f"Failed to create temporary file for {path}")
...
def _resolve_methodology_title(
    site_config: Mapping[str, Any],
    registry: Mapping[str, Any],
    lang: str,
    site_name: str,
    ...
    title = _resolve_multilingual_text(
        ...
        lang,
        candidate_paths=[
            ("seo", "pages", "methodology", "title"),
            ("seo", "methodology_title"),
    ...
    fallback_map = {
        "en": f"Methodology | {site_name}",
        "ar": f"المنهجية | {site_name}",
        "fr": f"Méthodologie | {site_name}",
        "es": f"Metodología | {site_name}",
        "de": f"Methodik | {site_name}",
        "zh": f"方法论 | {site_name}",
        "ja": f"方法論 | {site_name}",
    }
...
def _resolve_methodology_description(
    site_config: Mapping[str, Any],
    registry: Mapping[str, Any],
    lang: str,
    site_name: str,
    ...
    description = _resolve_multilingual_text(
        ...
        lang,
        candidate_paths=[
            ("seo", "pages", "methodology", "description"),
            ("seo", "methodology_description"),
            ("seo", "methodology", "description"),
        ],
        default="",
    )
    if description:
    ...
    fallback_map = {
        "en": f"Learn how {site_name} compares travel structures, constraints, tradeoffs, and traveler fit through a repeatable reference methodology.",
        "ar": f"تعرّف على كيفية مقارنة {site_name} لأشكال السفر والقيود والمفاضلات وملاءمة المسافر عبر منهجية مرجعية قابلة للتكرار.",
        "fr": f"Découvrez comment {site_name} compare les structures de voyage, les contraintes, les arbitrages et l'adéquation voyageur au moyen d'une méthodologie de référence répétable.",
        "es": f"Descubre cómo {site_name} compara estructuras de viaje, restricciones, compensaciones y adecuación del viajero mediante una metodología de referencia repetible.",
        "de": f"Erfahren Sie, wie {site_name} Reiseformen, Beschränkungen, Trade-offs und Reisepassung mit einer wiederholbaren Referenzmethodik vergleicht.",
        "zh": f"了解 {site_name} 如何通过可重复的方法论比较旅行结构、约束条件、权衡关系与旅客适配度。",
        "ja": f"{site_name} が再現可能な参照方法論によって旅行構造、制約、トレードオフ、旅行者適合性をどのように比較するかを説明します。",
    }
    return fallback_map.get(lang, fallback_map["en"])
...
def _build_language_switcher(
    ...
    for language in get_enabled_languages(site_config, registry=registry):
        ...
        if url is None:
            raise GenerateMethodologyError(f"Missing URL in language URL map for language: {code!r}")
...
def _build_methodology_seo(
    ...
    canonical_url = build_methodology_path(site_config, lang, absolute=True, registry=registry)
    urls_by_lang = build_static_language_url_map(site_config, "methodology", registry=registry)
...
def build_methodology_context(
    ...
    title = _resolve_methodology_title(site_config, registry, lang, site_name)
    description = _resolve_methodology_description(site_config, registry, lang, site_name)

    seo_payload = _build_methodology_seo(
        site_config,
        registry,
        lang,
        title,
    ...
    methodology_urls_by_lang = build_static_language_url_map(site_config, "methodology", registry=registry)
    ...
    context: Dict[str, Any] = {
        ...
        "methodology_urls_by_lang": methodology_urls_by_lang,
        "language_switcher": _build_language_switcher(site_config, registry, lang, methodology_urls_by_lang),

        # Generic aliases
        "page_lang_urls": methodology_urls_by_lang,
    }
...
def _load_methodology_template(env: Environment):
    try:
        return env.get_template("pages/methodology.html")
    except Exception as exc:
        raise RenderError(f"Unable to load template 'pages/methodology.html': {exc}") from exc
...
def render_methodology_html(
    ...
    *,
    ...
    template = _load_methodology_template(env)
    context = build_methodology_context(site_config, registry, lang)

    try:
        html = template.render(**context)
    except Exception as exc:
        raise RenderError(f"Failed to render methodology page for language {lang!r}: {exc}") from exc

    if not isinstance(html, str) or not html.strip():
        raise RenderError(f"Rendered methodology page is empty for language {lang!r}.")
...
def _resolve_target_languages(
    ...
    if requested_lang:
        ...
        if requested not in enabled_codes:
            raise GenerateMethodologyError(
...
def generate_methodology_pages(
    *,
    ...
    for lang in target_languages:
        rendered_by_lang[lang] = render_methodology_html(
            site_config,
            registry,
            lang,
            env=env,
    ...
    try:
        for lang, html in rendered_by_lang.items():
            output_path = output_dir / lang / "methodology" / "index.html"
            _atomic_write_text(output_path, html)
            written_paths.append(output_path)
            log.info("Generated methodology page [%s] -> %s", lang, output_path)
    except Exception as exc:
        raise GenerateMethodologyError(f"Failed while writing generated methodology page output: {exc}") from exc
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the multilingual TourVsTravel methodology page."
    )
    parser.add_argument(
        "--lang",
        type=str,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    except GenerateMethodologyError as exc:
        log.error(str(exc))
        return 1
    except Exception as exc:
        log.exception("Unexpected methodology generator failure: %s", exc)
        return 1

    log.info("Methodology generation completed successfully. Files written: %d", len(written))
    return 0
